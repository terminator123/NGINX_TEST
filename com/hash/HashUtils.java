...
public class HashUtils {
	...
	public static int getHashCode(String key,String hashname){
		//System.out.println(key + " &&& " + hashname);
		switch(hashname){
			case "crc32Hash":return crc32Hash(key);
			case "additiveHash":return additiveHash(key);
			case "rotatingHash":return rotatingHash(key);
			case "BKDRHash":return BKDRHash(key);
			case "FNVHash":return FNVHash(key);
			case "RSHash":return RSHash(key);
			case "JSHash":return JSHash(key);
			case "PJWHash":return PJWHash(key);
			case "ELFhash":return ELFhash(key);
			case "SDBMHash":return SDBMHash(key);
			case "DJBHash":return DJBHash(key);
			case "APHash":return APHash(key);
			default:return 0;
